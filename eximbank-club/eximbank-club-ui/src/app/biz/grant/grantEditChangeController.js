...
angular.module('app').controller(
	'grantEditChangeController', ['$scope', '$rootScope', '$state', '$http', 'toaster', '$CanvasService', '$filter','WebUploadService','$timeout',
		function($scope, $rootScope, $state, $http, toaster, $CanvasService, $filter,WebUploadService,$timeout) {
			...
			function getById(grantId) {
				...
				$.ajax({
					type: 'PUT',
					url: '/eximbank-club/grant/getGrant',
					data: angular.toJson({
						"grantId": grantId
					})
            ...
            //设置过期日期不可删除
            $scope.addDisabled = function(compareDate) {
                var dt = new DateUtil();
                var now = dt.nowTime('yy-mm-dd'); //系统日期（当天)
                if(dt.compareDate(compareDate,now) == 1){
                    return true;
                }
                return false;
            }
			...
			function initData() {
				...
				if(policyLendingStatus==''){
					policyLendingStatus = '2';
				}
			...
			function validateFlied() {
				var ck = $scope.checkScopeBnsPeriod();
				if(!ck) {
					alert("业务期限不合法!");
					return false;
				}
