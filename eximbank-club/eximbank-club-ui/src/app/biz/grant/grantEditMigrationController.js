...
angular.module('app').controller(
	'grantEditMigrationController', ['$scope', '$rootScope', '$state', '$http', 'toaster', '$CanvasService', '$filter', 'WebUploadService', '$timeout',
		function($scope, $rootScope, $state, $http, toaster, $CanvasService, $filter, WebUploadService, $timeout) {
			...
			function getById(grantId) {
				...
				$.ajax({
					type: 'PUT',
					url: '/eximbank-club/migratioin/getGrant',
					data: angular.toJson({
						"grantId": grantId
					})
			...
			function initData() {
				...
				for(var i = 0; i < chargeTypeList.length; i++) {
					...
					var rt_val = convertBlankOrEmpty(rateValueList[i]);
					$("input[name='rateValue']").eq(i).val(rt_val == 0 ? '' : rt_val);
				}
				//币种
             ...
             * 功能：判断费用类型是否可新增
             */
            function currencyReadOnly() {
				//判断币种是否可修改
				var size=$("select[name='currency']").find("option").size();
				//如果没有其他费用，则不可添加
				if(size < 3) {
					$(".cnyBtn").hide();
				}
			...
			$scope.checkSendDate = function() {
				...
				if(dt.compareDate(sendDt, efeeDate) == 1) {
					...
					return false;
				} else if(dt.compareDate(sendEndDate, sendDt) == 1) {
			...
			$scope.checkRateValue = function($event) {
				...
				if(rateRangeMix == null || rateRangeMix == '' || rateRangeMix == 'null') {
					rateRangeMix = 0;
				}
				if(rateRangeMax == null || rateRangeMax == '' || rateRangeMax == 'null') {
					rateRangeMax = 1000;
				}
				//有折扣
			...
			$scope.submit = function() {
				...
				if(submitCheck() && validateFlied() && m) {
					...
					getZzxx();
					//提交前进行确认
					confirm("提示", "请确认所有数据项信息及附件材料，提交后将不允许修改!",
						function(flag) {
							if(!flag) {
								$scope.loading = false;
								return false;
							...
							$.ajax({
								url: '/eximbank-club/migratioin/save',
								type: 'POST',
				...
				function callback(result) {
					...
					if(result.httpCode == 200) { //成功
						alert("保存成功");
						setTimeout(function() {
							$state.go('main.biz.grant.migratioin');
						}, 1000);
					} else {
						alert(result.msg);
			...
			$scope.tempSave = function(vtype) {
				...
				function callback(result) {
					if(result.httpCode == 200) { //成功
						...
						setTimeout(function() {
							//提交后转到数据迁移暂存列表
							$state.go('main.biz.grant.queryHistory');
						}, 1000);
					} else {
						alert(result.msg);
					}
			...
			$scope.guaranteeAdd = function(code) {
				...
				//更新缓存中的内容
				//              $scope.record.bizGuaranteeContractList.splice(i,1,record);
			};
			//担保合同数组删除指定的元素
