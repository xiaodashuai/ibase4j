...
angular.module('app').controller(
	'grantAddController', ['$scope', '$rootScope', '$state', '$http', 'toaster', '$CanvasService', '$filter', 'WebUploadService','$timeout',
		function($scope, $rootScope, $state, $http, toaster, $CanvasService, $filter, WebUploadService,$timeout) {
			...
			$scope.changeTab = function(tab) {
				var promise = $CanvasService.createCanvas(angular.element('.canvas')[0], "B", angular.element('.canvasNum').find('.active').attr("num"), $scope.gantRuleVerifVo.grantCode, null, $scope, tab);
			...
			//获取登录用户信息
			function getLoginUser(code) {
				$scope.loading = true;
				$.ajax({
					url: '/eximbank-club/bizRentalFactoring/read/detail',
					type: 'PUT',
					data: angular.toJson({
						'debtCode': code
					})
					...
					if(result) {
						if(result.httpCode == 200) {
							...
							$scope.record.descriptionProgramQuoqate = data.descriptionProgramQuoqate;
							$scope.record.descriptionRateRules = data.descriptionRateRules;
							$scope.record.policy = data.policy;
							$scope.record.policyDescription = subStrLen(data.policyDescription,500);
							$scope.record.iouCode = data.iouCode;
							$scope.custUserLetter = data.custUserLetter;
							$scope.record.agent = data.attn;
							$scope.record.agencies = data.attnOrg;
							$scope.record.gracePeriod=0;
							$scope.backgroundNationality = data.backgroundNationality;
							$scope.industryInvestment = data.industryInvestment;
							//加载数据
							getByCode(debtNum);
							//查询约束条件
				...
				$.ajax({
					...
					if(result) {
						if(result.httpCode == 200) {
							...
							var bnval = $scope.backgroundNationality;
							var investment = $scope.industryInvestment;
							//根据产品编码查询产品名称
							var pTypes = getProductTypes(businessCode);
							$scope.model.businessName = pTypes.name;
							//初始化下拉框
							initSelectBox(businessCode, debtNum, bnval, investment);
							//设置利率类型 
							changeRateType($("select[name='rateType']").eq(0));
						} else {
							$scope.msg = result.msg;
						}
					} else {
				...
				$.ajax({
					...
					if(result) {
						if(result.httpCode == 200) {
							...
							var remDate = $scope.gantRuleVerifVo.expiDate;
							//方案费率类型,如果等于0说明没有费率
							...
							var nowDat = new Date();
							var days = (new Date(remDate)).getTime() - nowDat.getTime();
							var time = parseInt(days / (1000 * 60 * 60 * 24));
                			//提醒有效期
							alert("提示", "方案还有" + time + "天失效，请在失效日期前完成放款");
							//延迟0.2秒执行
							setTimeout(function() {
								//判断币种是否可修改 
								cnyReadOnly();
			...
			/**
			 * 验证发放日期是否合适，发放日期，不可在方案失效日期之后，不可在系统日期之前；必须在生效日期之后
			 */
...
//              }
			...
			$scope.submit = function() {
				$scope.loading = true;
				//提交的时候也需要快照
				var promise = $CanvasService.createCanvas(angular.element('.canvas')[0], "B", angular.element('.canvasNum').find('.active').attr("num"), $scope.gantRuleVerifVo.grantCode, null, $scope, 4);
				promise.then(function(data) {
					$scope.tab = 4;
				}, function(data) {
					console.log("创建快照失败 error data ==" + data);
				...
				if(submitCheck() && validateFlied() && m) {
					...
					$scope.record.businessCode = businessCode;
					$scope.record.businessName = $scope.model.businessName;
					$scope.record.debtCode = debtNum;
					$scope.record.custNo = $scope.model.createBy;
					$scope.record.grantCode = $scope.gantRuleVerifVo.grantCode;
					$scope.record.bizGuaranteeContractList = splitContractArray(bizGuaranteeContractList);
			...
			$scope.tempSave = function(vtype) {
				...
				if(m) {
					if('dv1' == vtype) {
						...
						$scope.record.grantCode = $scope.gantRuleVerifVo.grantCode;
						$scope.record.businessName = $scope.model.businessName;
						$scope.record.bizGuaranteeContractList = bizGuaranteeContractList;
				...
				function callback(result) {
					if(result.httpCode == 200) { //成功
						alert("提示", "保存成功");
						setTimeout(function() {
							$state.go('main.biz.grant.queryTemp');
						}, 1000);
					} else {
						alert("提示", result.msg);
					}
			...
			$scope.payListChange = function() {
				...
				var names = document.getElementsByName('synchrRefreshPayList');
				names[0].value = "yes";
				cleanRequiredWarn(names[0]);
			}
			//校验末次还息日期
			...
			$scope.chgAlleviationLoan = function(){
				...
				if(ck == 2){
					...
					//产业精准扶贫贷款带动人数-去掉必输以及验证信息
					$("#loanServicePeopleNum").addClass("required");
					//精准扶贫贷款资金来源-去掉必输以及验证信息
					$("#loanFunResource").addClass("required");
					//精准扶贫主体性质-去掉必输以及验证信息
					$("#povertyProperty").addClass("required");
				}
			}
			/**
			 * 切换是否银团
			...
			//担保合同初始化
			var bizGuaranteeContractList = new Array();
			/**
			 * 担保合同添加
			 * @param {Object} code
			 */
			$scope.guaranteeAdd = function(code) {
				//数组长度
				var size = bizGuaranteeContractList.length;
				var record = new Object();
				//如果数组内有元素修改，修改的原理是首先删除元素，然后再新增进数组
				if(size > 0) {
					var k = getGuaranteeIndexByCode(bizGuaranteeContractList,code);
					if(k!=-1){
						guaranteeDel(k);
					}
				}
				...
				bizGuaranteeContractList.push(record);
			};
			//担保合同数组删除指定的元素
			function guaranteeDel(i) {
				...
				bizGuaranteeContractList.splice(i, 1);
			};
			
			//从索引位置获取数组指定的元素
			function guaranteeGet(code) {
				var size = bizGuaranteeContractList.length;
				...
				return getGuaranteeByNo(bizGuaranteeContractList,code);
			}
...
/**
 * 功能：初始化下面各类下拉框（动态生成） 包括货币、中国制造2025及战略新兴产业分类、支农投向等
 * @param {Object} businessCode
 * @param {Object} debtNum
 * @param {Object} country 
 * @param {Object} investment
 */
function initSelectBox(businessCode, debtNum, country, investment) {
	//行业投向
	...
	//延迟0.1秒执行
	setTimeout(function() {
		initData(businessCode,investment,country);
	}, 100);
}
/**
 * 初始化需要从方案带过来的值
 * @param {Object} businessCode
 * @param {Object} investment 行业投向编码
 * @param {Object} country
 */
function initData(businessCode,investment,country){
	//国别回显
	$("#backgroundNationality").find("option[value='" + country + "']").attr("selected", true);
	//设置下拉框选中的名称
	$("#compareUL").find(".defaultDownBox").text("非支农贷款行业");
	//设置选中的值赋值给隐藏表单
	$("#compareUL").prev().val("A000");
	//设置对外投资贷款分类
	if(businessCode=="501000000000" ){
	    $("#investmentLoanCkassifcationUL").find(".defaultDownBox").text("有追索权国内保理");
	    //设置选中的值赋值给隐藏表单
	    $("#investmentLoanCkassifcationUL").prev().val("05600210");
	}else {
	    $("#investmentLoanCkassifcationUL").find(".defaultDownBox").text("无追索权国内保理");
	    //设置选中的值赋值给隐藏表单
	    $("#investmentLoanCkassifcationUL").prev().val("05600220");
	}
	//默认选中值
    $("#businessTypeUL").find(".defaultDownBox").text("其他租金保理");
    $("#businessTypeUL").prev().val("9000890");
	//默认选中值
    $("#innovativeBusinessTypeUL").find(".defaultDownBox").text("其他租金保理");
    $("#innovativeBusinessTypeUL").prev().val("0433");
    //行业投向从方案带过来后回显
    $.when(getTypeDetailDb(investment,'005')).done(function(result){
		if(result !=null){
			var name = result.name;
			if(name && name != '' && name!=null){
				$("#industryInvestmentUL").find(".defaultDownBox").text(name);
    			$("#industryInvestmentUL").prev().val(investment);
			}
		}
	}).fail(function(){
		alert("查询失败!");
 ...
 * @param {Object} b 元素id属性
 */
function changeGuaranteeInfoSelect(b) {
	var parentCode = $(b).val();
	if(!parentCode) {
		return;
	}
...
function getGuaranteeChildrenList(b) {
    ...
    if(code != '') {
        ...
        if('C000'==guaranType){
            ...
            table.find("input[name='guaranteeNo']").eq(0).parent().parent().hide();
            //担保合同编号修改赋值逻辑
        ...
        //信用类型C000和其他担保类型C104可显示，其余的担保方式类型是不可显示
        if('C104'==guaranType||'C000'==guaranType){
        	...
        	//,保证C101、抵押C102、质押C103担保方式类型下拉框初始化数据
        ...
        //基础担保类型为抵押C102、质押C103时，需要录入押品信息
