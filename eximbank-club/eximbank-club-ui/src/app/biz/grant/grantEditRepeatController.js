...
angular.module('app').controller(
	'grantEditRepeatController', ['$scope', '$rootScope', '$state', '$http', 'toaster','$CanvasService', '$filter','WebUploadService','$timeout',
		function($scope, $rootScope, $state, $http, toaster, $CanvasService, $filter,WebUploadService,$timeout) {
				...
				$.ajax({
					...
					if(result) {
						if(result.httpCode == 200) {
							...
							//判断是否发放变更
							var oldGrantCode = $scope.record.originalGrantCode;//原发放编码
							isChangeDisabled(oldGrantCode!=null);
							//初始化下拉框
			...
			//变更业务指定的元素不可修改
			function isChangeDisabled(d){
				if(d == true){
					//1.管理信息全部元素不可用
					//所有输入框不可用
					$("#dv1").find("input").prop("disabled",true);
					//所有选择框不可用
					$("#dv1").find("select").prop("disabled",true);
					//所有文本域不可用
					$("#dv1").find("textarea").prop("disabled",true);
					//所有隐藏表单不可用
					$("#dv1").find("hidden").prop("disabled",true);
					//是否银团代理行
					$scope.isSyndicatedAgencyYes=true;
					
					//2.发放条件部分元素不可用
					$("#sendDate").prop("readonly","readonly");//发放开始日期
					$("#sendEndDate").prop("readonly","readonly");//发放截至日期
					$("#dv2").find("select[name='currency']").prop("disabled",true);//币种
					$("#dv2").find(".cnyBtn").hide();//新增币种按钮不显示
					$("#dv2").find("select[name='rateType']").prop("disabled",true);//利率类型
					$("#dv2").find("input[name='rateVal']").prop("readonly","readonly");//利率
					$("#dv2").find("select[name='irBk']").prop("disabled",true);//利率基准
					$("#dv2").find("select[name='floatDirectioin']").prop("disabled",true);//利率浮动方向
					$("#dv2").find("select[name='floatMode']").prop("disabled",true);//利率浮动方式
					$("#dv2").find("input[name='floatingRate']").prop("readonly","readonly");//浮动率
					$("#dv2").find("input[name='varCycle']").prop("readonly","readonly");//利率变动周期
					$("#dv2").find("select[name='chgCycleUnit']").prop("disabled",true);//变动周期单位
					$("#dv2").find("select[name='overdueRateCalcMode']").prop("disabled",true);//逾期利率计算方式
					$("#dv2").find("input[name='overdueIncrRatio']").prop("readonly","readonly");//逾期加点(或百分比)
					$("#dv2").find("input[name='calcDays']").prop("readonly","readonly");//利率计算天数
					$("#dv2").find("select[name='paymentMode']").prop("disabled",true);//支付方式
					$("#dv2").find("input[name='guaranteeNo']").removeProp("readonly");//担保合同编号允许修改
					$("#dv2").find("textarea[name='guaranRemark']").removeProp("readonly");//担保合同备注允许修改
					
					//3.专有信息所有输入框不可用
					$("#dv3").find("input").prop("disabled",true);
					//所有选择框不可用
					$("#dv3").find("select").prop("disabled",true);
					//4.还款计划
					$("#fristRepayDate").prop("readonly","readonly");//首次还本日
					$("#lastRepayDay").prop("readonly","readonly");//末次还款日
					$("#repaymentCostMode").prop("disabled",true);//还本方式
					$("#firstDay").prop("readonly","readonly");//首次还息日
					$("#caclWay").prop("disabled",true);//利息计收方式
				}
			}
			/**
			 * 切换是否精准扶贫贷款
			...
			function initData() {
				...
				for(var i = 0; i < chargeTypeList.length; i++) {
					...
					$("input[name='rateValue']").eq(i).val(rateValueList[i]);
				}
				//币种
				...
				for(var i = 0; i < currencyList.length; i++) {
					...
					$("input[name='convertedPrice']").eq(i).val(convertedPriceList[i]);
					$("input[name='paymentAmt']").eq(i).val(paymentAmtList[i]);
					$("select[name='rateType']").eq(i).val(rateTypeList[i]);
					//如果是浮动利率，利率输入框为只读模式
					changeRateType($("select[name='rateType']").eq(i));
					$("input[name='rateVal']").eq(i).val(rateValList[i]);
					...
					$("input[name='floatingRate']").eq(i).val(floatingRateList[i]);
					$("input[name='varCycle']").eq(i).val(varCycleList[i]);
					$("select[name='chgCycleUnit']").eq(i).val(chgCycleUnitList[i]);
					$("select[name='overdueRateCalcMode']").eq(i).val(overdueRateCalcModeList[i]);
					$("input[name='overdueIncrRatio']").eq(i).val(overdueIncrRatioList[i]);
					$("input[name='overdueIncrRatio']").eq(i).val(overdueIncrRatioList[i]);
					$("input[name='calcDays']").eq(i).val(convertBlankOrEmpty(calcDaysList[i])==''?360:calcDaysList[i]);
					$("select[name='paymentMode']").eq(i).val(paymentModeList[i]);
				}
				//下面是担保信息回写
...
//              }
			...
			$scope.submit = function() {
				...
				if(submitCheck() && validateFlied() && m) {
					...
					$.ajax({
						url: '/eximbank-club/grant/update',
						type: 'POST',
