'use strict';
    ...
    angular.module('app')
        .controller('blProcessCZController', ['$scope', '$rootScope', '$state', '$timeout', 'toaster','$CanvasService','WebUploadService',
        ...
        var adid = $state.params.adid;
         ...
         function locking() {
             $.ajax({
                 ...
                 data: angular.toJson({'debtCode':debtCode,'code':debtCode})
             }).then(function(result) {
                 $scope.loading = false;
                 if (result.httpCode == 200) {
         ...
         $scope.goBack= function(){
             ...
             $.ajax({
                 ...
                 data: angular.toJson({'code':debtCode})
             }).then(function(result) {
                 $scope.loading = false;
                 if (result.httpCode == 200) {
                     history.back(-1);
         ...
         $scope.submit= function(){
             ...
             promise.then(function(data) {
                 ...
                 if($scope.comment.name=="通过"){
                     var commite="TT";
                     var invokeInf  = 1;
                     paramsJson.invokeInf = invokeInf;
                 }else if($scope.comment.name=="驳回"){
                     var commite="BLBH";
                 ...
                 paramsJson.updateDebt = updateDebt;
                 paramsJson.trn = "main";
                 paramsJson.process ="FASH";
                 var m = $scope.record;
                 if(m){
