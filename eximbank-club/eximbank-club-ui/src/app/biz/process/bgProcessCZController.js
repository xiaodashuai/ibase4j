...
angular.module('app')
    .controller('bgProcessCZController', ['$scope', '$rootScope', '$state', '$timeout', 'toaster','$CanvasService','WebUploadService',
        function($scope, $rootScope, $state, $timeout, toaster,$CanvasService,WebUploadService) {
            ...
            $scope.submit= function(){
                angular.element('#refer').attr("disabled",true);
                if($scope.record.zjaff3==null || $scope.record.zjaff3==''){
                ...
                promise.then(function(data) {
                    ...
                    var processParams = {}
                    $scope.comment="";
                    if($scope.record.zjaff2==1 && $scope.record.zjaff3=="通过"){
                        $scope.comment = $scope.transitions[0];
                    }
                    if($scope.record.zjaff2==2){
                        $scope.comment = $scope.transitions[1];
                    }
                    if($scope.record.zjaff3=="驳回"){
                        $scope.comment = $scope.transitions[2];
                    }
                    ...
                    if($scope.record.zjaff1==null || $scope.record.zjaff1==""){
                        alert("","'业务变更后符合原方案' 选项 请选择");
                        angular.element('#refer').removeAttr("disabled");
                        return false;
                    }
                    if($scope.record.zjaff1==1 ){
                        if($scope.record.zjaff2==null || $scope.record.zjaff2==""){
                            alert("","'是否属于非责任性变更' 选项 请选择");
                            angular.element('#refer').removeAttr("disabled");
                            return false;
                        }
                    }
