angular.module('app')
	.controller('schemeTemContinueController', ['$scope', '$rootScope', '$http', '$state', '$stateParams', '$timeout', '$CanvasService', 'WebUploadService',
		function($scope, $rootScope, $http, $state, $stateParams, $timeout, $CanvasService, WebUploadService) {

			$scope.loading = false;
			//得到债项概要编号
			var debtCode = $state.params.bizcode;
			var statee = $state.params.statee;
			var debtCodeKuaiZhao = debtCode;
			//用于判断是补录和修订
			var debtCode2 = debtCode.substring(0, 13);
			var debtCode3 = debtCode.substring(13);
			debtCode3 = Number(debtCode3);
			if(statee == '2') {
				//处理快照要用的debtCode
			...
			function getAllSchemeInformation() {
				...
				$.ajax({
					...
					if(result.httpCode == 200) {
						...
						angular.forEach($scope.rentFacList, function(v1, k1) {
							//处理承租人的下拉款展示
							v1.hiddenRecordProInform = false;
							ccussTem = v1.custNo;
