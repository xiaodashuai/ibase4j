...
/*function setCheckedNode(){
	...
	var node = zTree.getNodeByParam("id",checkDeptId);
	//如果是修改
	if(node){
...
function getZNode(n_id, n_pid, n_name, n_open, n_nocheck) {
	return {id:Number(n_id),pId:Number(n_pid),name:n_name,open:true,nocheck:false};
}