$(function() {
	$("li.dropdown-submenu a.dropdown-toggle").on("click", function(g) {
		g.preventDefault();
		g.stopPropagation();
	...
	var a = 0;
	$("#content .submenu > *").not(".collapsed").each(function() {
		a += $(this).width()
	});
	var e = function() {
		if($(window).width() < (a + 285)) {
	...
	var b = function() {
		if($(window).width() < 768) {
			var g = $(".navbar-collapse .collapsing-content").html();
			$(".navbar-collapse .collapsing-content").hide();
			$(".collapsed-content").html(g);
			if($("body").hasClass("direction-rtl")) {
				var h = "left"
			} else {
				var h = "right"
			...
			$("#color-schemes li a").click(function() {
				...
				var j = $("body").attr("class").split(" ").pop();
				$("body").removeClass(j).addClass(i)
			})
		} else {
	...
	$(".quick-action.dropdown, .submenu .nav .dropdown, .messages.dropdown, .settings.dropdown, .change-status.dropdown").on("show.bs.dropdown", function(g) {
		$(this).find(".dropdown-menu").addClass("animated fadeInDown");
		$(this).find("#user-inbox").addClass("animated bounceIn")
	});
	$("#navigation .dropdown").on("show.bs.dropdown", function(g) {
		$(this).find(".dropdown-menu").addClass("animated fadeInDown")
	});
	$("#user-status .dropdown li a").click(function() {
		var g = $(this).data("status");
		var i = $("#user-status").attr("class").split(" ").pop();
		var h = $(this).html();
		$("#user-status").removeClass(i).addClass(g);
		$("#user-status .my-status").html(h)
	});
	$("#color-schemes li a").tooltip({
		placement: "bottom",
		trigger: "hover",
	...
	$("#color-schemes li a").click(function() {
		...
		var h = $("body").attr("class").split(" ").pop();
		$("body").removeClass(h).addClass(g)
	});
