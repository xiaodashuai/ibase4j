...
angular.module('app')
    .controller('updateController', ['$scope', '$rootScope', '$state', '$timeout', 'toaster',
        function ($scope, $rootScope, $state, $timeout, toaster) {
            ...
            if ($state.includes('**.msgConfig.update')) {
                title = "模板修改";
            ...
            } else if ($state.includes('**.msgConfig.create')) {
                title = "模板新增";
                getOrgSelectBox();
                validate();
            }
            ...
            $scope.submit = function () {
                ...
                if (m) {
                    ...
                    $.ajax({
                        url: '/eximbank-console/msgConfig/save',
                        type: 'POST',
                ...
                function callback(result) {
                    if (result.httpCode == 200) { //成功
                        ...
                        $timeout(function () {
                            $state.go('main.sys.msgConfig.list');
                        }, 2000);
                    } else {
                        toaster.clear('*');
            ...
            function activate(id) {
                $scope.loading = true;
                $.ajax({
                    url: '/eximbank-console/msgConfig/read/detail',
                    type: 'POST',
                    data: angular.toJson({
                        'id': id
                    })
