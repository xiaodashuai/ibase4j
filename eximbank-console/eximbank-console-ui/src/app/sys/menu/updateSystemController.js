...
angular.module('app')
    .controller('menuUpdateSystemController', ['$scope', '$rootScope', '$state', '$timeout', 'toaster', '$stateParams',
        function ($scope, $rootScope, $state, $timeout, toaster, $stateParams) {
            var title = "";
            var menuname = $stateParams.menuName;
            $scope.myImage = '';
            $scope.myCroppedImage = '';
            if ($state.includes('**.menu.updateSystem')) {
                title = "后台菜单修改";
                ...
                selectMenuType();
                validate();
            } else if ($state.includes('**.menu.addSystem')) {
                title = "后台菜单新增";
                //add by zhy  20180207 显示上级菜单信息
                selectMenuType();
