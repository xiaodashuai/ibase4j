...
angular.module('app')
	.controller('countryRiskRatingController', ['$rootScope', '$scope', '$http', '$state',
		function ($rootScope, $scope, $http, $state) {
			$scope.title = '国别风险等级查询';
			$scope.param = {};
			$scope.loading = false;
			...
			$scope.search = function () {
				$scope.loading = true;
				$.ajax({
					url: '/eximbank-console/countryRiskRating/read/list',
					type: 'POST',
			...
			$scope.edit = function () {
				...
				$state.go('main.biz.countryRiskRating.detail', {
					id: item.id,
					classifyid: item.riskLevel
				});
			}
			// 翻页
