...
public final class BizWebUtil {
	...
	public static final void saveCurrentUser(Object user) {
		setSession(BizContant.BIZ_CURRENT_USER, user);
	}
	
	/** 保存当前用户对象 */
	public static final void saveCurrentUserObject(SysUser user) {
		setSession(BizContant.BIZ_CURRENT_USER_OBJ, user);
	}

	/** 保存当前用户 */
	public static final void saveCurrentUser(HttpServletRequest request, Object user) {
		setSession(request, BizContant.BIZ_CURRENT_USER, user);
	}
	
	/** 获取当前用户 */
	public static final SysUser getCurrentUserObject() {
		Subject currentUser = SecurityUtils.getSubject();
		if (null != currentUser) {
			try {
				Session session = currentUser.getSession();
				if (null != session) {
					return (SysUser) session.getAttribute(BizContant.BIZ_CURRENT_USER_OBJ);
				}
	...
	public static final Long getCurrentUser() {
		...
		if (null != currentUser) {
			try {
				Session session = currentUser.getSession();
				if (null != session) {
					return (Long) session.getAttribute(BizContant.BIZ_CURRENT_USER);
				}
			} catch (InvalidSessionException e) {
				logger.error(e);
			}
	...
	public static final Object getCurrentUser(HttpServletRequest request) {
		try {
			...
			if (null != session) {
				return session.getAttribute(BizContant.BIZ_CURRENT_USER);
			}
		} catch (InvalidSessionException e) {
			logger.error(e);
		}
	...
	public static final void removeCurrentUser(HttpServletRequest request) {
		request.getSession().removeAttribute(BizContant.BIZ_CURRENT_USER);
	}
