...
public final strictfp class MathUtil {
	...
	/**
	 * 功能：金钱数字保留2位小数且三位三位的隔开
	 * @param money
	 * @return 如果金额为空，则返回空字符串，否则返回格式化的金额,2位精度
	 */
	public static final String formatMoney(BigDecimal money){
		if(money!=null){
	 ...
	 * 格式化数字，前面补零(0)
	 * @param number 数值
	 * @param len 需要补齐的数字字符串总长度
	 * @return
	 */
	public static String formatZeroInt(int number, int len){
		StringBuffer format = new StringBuffer();
		for(int i=0;i<len;i++){
			format.append("0");
		}
		return formatNumber(Integer.valueOf(number), format.toString());
	}
	...
	/**
	 * 功能：把整数除以100后转换成小数，精确度是两位小数，保留2位
	 * @param num 百分比
	 * @return 返回百分比转换的整数
	 */
	public static float convertIntegerToPercentFloat(int num){
		if(num!=0){
			Double dReult = divide(num,100);
			BigDecimal bdResult = bigDecimal(Double.valueOf(dReult));
			return bdResult.setScale(2, BigDecimal.ROUND_HALF_UP).floatValue();
		}else{
			return 0.00f;
		}
	}
	...
	public static String formatToNumber(BigDecimal obj){
		...
		if(obj.compareTo(BigDecimal.ZERO)==0){
			   ...
			   amt="0"+df.format(obj).toString();
		}else {
			   amt=df.format(obj).toString();
		}
		String amt1=amt.substring(0,amt.length()-3);
		String amt2=amt.substring(amt.length()-2);
		String newAmt=amt1+amt2;
		return newAmt;
