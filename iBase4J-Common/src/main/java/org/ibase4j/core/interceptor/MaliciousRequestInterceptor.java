...
public class MaliciousRequestInterceptor extends BaseInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		response.setHeader("Access-Control-Allow-Headers",
				"Origin,x-requested-with,Content-Type, Accept,If-Modified-Since,Access-Control-Allow-Origin,EX-SysAuthToken,EX-JSESSIONID");
		...
		if (logger.isDebugEnabled()) {
			logger.debug("MaliciousRequestInterceptor is preRequest={}", preRequest);
			logger.debug("MaliciousRequestInterceptor is preRequestTime={}", preRequestTime);
		}
